...
/**
 * 可以通过构造方法传入身份证号，也可以通过set/get访问器进行
 * 传入身份证号，然后调用isCorrect方法进行验证身份证号，然后
 * 调用getErrMsg方法显示验证结果。
 * 例如：
 * IdCard idcard = new IdCard("513723205506161215");
 * idcard.isCorrect();
 * System.out.println(idcard.getErrMsg());
 *
 */
public class IdCardUtil {
     ...
     * @param idCardNum
     */
    public IdCardUtil(String idCardNum,String hintName) {
       // super();
        this.idCardNum = idCardNum.trim();
        if(hintName!=null&&!hintName.equals("")){
        	errMsg = new String[] {hintName+"完全正确！", 
            		hintName+"为空！", 
            		hintName+"长度不正确！", 
            		hintName+"有非法字符！", 
            		hintName+"中出生日期不合法！", 
    ...
    private boolean isWordCorrect(){
    	...
    	if(this.isLenCorrect()){
            ...
            String regex = "^[0-9]";
            if(this.is15()){
            	for(int i=0; i<this.idCardNum.length(); i++){
            		if(!this.idCardNum.substring(i, i+1).matches(regex)){
            			iRet=false;
            			break;
            		}
            	}
            ...
            if(this.is18()){
            	for(int i=0; i<this.idCardNum.length(); i++){
            		String regex1 = "^[0-9,xX]";
            		if(i!=(this.idCardNum.length()-1)&&!this.idCardNum.substring(i, i+1).matches(regex)){
            			iRet=false;
            			break;
            		}else if(i==(this.idCardNum.length()-1)&&!this.idCardNum.substring(i, i+1).matches(regex1)){
            			iRet=false;
            			break;
            		}
            	}
